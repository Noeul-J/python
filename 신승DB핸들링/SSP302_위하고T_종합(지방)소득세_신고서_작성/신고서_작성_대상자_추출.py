...
def Get_Customer_To_Report():
    ...
    try:
        with conn.cursor() as curs:
            branches = "'강남','세무톡'"
            targetSql = "SELECT tg.CSTID, cst.CSTNAME, cst.RESIDENT_ID, rt.DUTY_TYPE, rt.EXP_RATIO FROM TB100023 AS tg LEFT JOIN TB100020 AS cst ON tg.CSTID=cst.CSTID LEFT JOIN TB100022 AS ad ON tg.CSTID=ad.CSTID LEFT JOIN TB300010 AS rt ON tg.CSTID=rt.CSTID WHERE IFNULL(tg.WehagoBookMake, '')='R' AND ad.CST_TYPE_YEAR='2021' AND IFNULL(CODE_TO_STR(ad.REG_BRANCH), '') IN ("+branches+");"
            ...
            if tgRow != None :
                tgCstId = tgRow[0]
                statusSql = "UPDATE TB100023 SET WehagoBookMake='I' WHERE CSTID='"+str(tgCstId)+"';"
                curs.execute(statusSql)
                conn.commit()
                
                totalIncomeSql = "SELECT SEC_CODE, AMOUNT_PAID, SIM_RATIO_N FROM TB300020 WHERE CST_TYPE_YEAR='2021' AND CSTID='"+str(tgCstId)+"';"
